class BudgetSerializer < ActiveModel::Serializer
  ...
  def earnings
    earnings = Earning.where(budget_id: self.object.id).order(amount: :desc)
    earnings_object = earnings.map {|earning| EarningSerializer.new(earning).as_json}
    earnings_object
  end

  def expenses
    expenses = Expense.where(budget_id: self.object.id).order(amount: :desc)
    expenses_object = expenses.map {|expense| ExpenseSerializer.new(expense).as_json}
    expenses_object
  end

  def bills
    bills = Bill.where(budget_id: self.object.id).order(amount: :desc)
